# Using the IDEAL POINT decision rule, calculates option
...
def idealPoint(matrix, weights):
    ...
    ideal = []
    nadir = []
    columns = numpy.transpose(matrix)
    for col in columns:
        ideal.append(max(col))
        nadir.append(min(col))
    for row in matrixlist:
        i = 0
        separIdeal = 0.0
        separNadir = 0.0
        for criteria in row:
            separIdeal = separIdeal + pow(weights[i] * (criteria - ideal[i]), 2)
            separNadir = separNadir + pow(weights[i] * (criteria - nadir[i]), 2)
            i = i + 1
        separIdeal = math.sqrt(separIdeal)
        separNadir = math.sqrt(separNadir)
        score = separNadir / (separNadir + separIdeal)
        scores.append(score)
    return scores
...
scores = idealPoint(table, weights)
